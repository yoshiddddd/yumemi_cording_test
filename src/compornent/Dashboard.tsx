...
export const Dashboard = () => {
  ...
  const [prefectureData, setPrefectureData] = useState<
    Record<number, PopulationCompositionWithLabel[]>
  >({});
  const [selectedKey, setSelectedKey] = useState<number>(0);
  const [showPrefectureData, setShowPrefectureData] = useState<
    PopulationCompositionWithLabel[][]
  >([]);
  ...
  useEffect(() => {
    const newShowPrefectureData = selectedPrefectures
  ...
  return (
    <>
      ...
      <Chart
        ...
        showPrefectureData={showPrefectureData}
        selectedKey={selectedKey}
      />
    </>
  );
