...
export const Chart = (props: ChartProps) => {
  const { selectedPrefectures, prefectures, showPrefectureData, selectedKey } =
    props;
  const combinedData =
    showPrefectureData[0] &&
    showPrefectureData[0][selectedKey].data.map((item, index) => {
      const newItem: { [key: string]: number } = { year: item.year };
      showPrefectureData.forEach((dataArray, dataIndex) => {
        newItem[`value${dataIndex}`] =
          dataArray[selectedKey].data[index]?.value || 0;
      });
      return newItem;
    });
  if (!combinedData) {
      ...
      >
        ...
        {showPrefectureData.map((_, index) => (
          <Line
            ...
            dataKey={`value${index}`}
            name={
              prefectures.find((p) => p.prefCode === selectedPrefectures[index])
                ?.prefName || `Series ${index + 1}`
            }
